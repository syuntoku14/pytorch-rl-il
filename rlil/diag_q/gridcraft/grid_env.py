...
class GridEnv(gym.Env):
    def __init__(self, gridspec,
        ...
        dS = self.num_states
        dA = self.num_actions
        self.transition_matrix = np.zeros((dS, dA, dS))
        for s in range(dS):
            for a in range(dA):
                tprobs = self.get_transitions(s, a)
                for next_s in tprobs:
                    self.transition_matrix[s, a, next_s] = tprobs[next_s]

        # build reward matrix
        self.rew_matrix = np.zeros((dS, dA, dS))
        for s in range(dS):
            for a in range(dA):
                for ns in range(dS):
                    self.rew_matrix[s, a, ns] = self.rew_fn(self.gs, s, a, ns)
    ...
    def step_stateless(self, s, a, verbose=False):
        ...
        if verbose:
            print('Act: %s. Act Executed: %s' %
                  (ACT_TO_STR[a], ACT_TO_STR[samp_a]))
